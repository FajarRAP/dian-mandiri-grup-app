...
import 'package:ship_tracker/features/auth/domain/entities/sign_in_entity.dart';
...
void main() {
  ...
  group('fetch user repository test', () {
    ...
    test(
      'should return Right(UserEntity) when remote call is successful',
      () async {
        // arrange
        when(
          () => mockAuthRemoteDataSource.fetchUser(),
  ...
  group('fetch user from storage repository test', () {
    ...
    test(
      'should return Right(UserEntity?) when local call is successful',
      () async {
        // arrange
        when(
          () => mockAuthLocalDataSource.getUser(),
  ...
  group('refresh token repository test', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          () => mockAuthRemoteDataSource.refreshToken(params),
        ).thenAnswer((_) async => resultMatcher);
        when(
          () => mockAuthLocalDataSource.cacheTokens(
  ...
  group('sign in repository test', () {
    ...
    test(
      'should return Right(SignInEntity) when remote call is successful',
      () async {
        // arrange
        when(
        ...
        when(
          () => mockAuthLocalDataSource.cacheUser(user: any(named: 'user')),
        ).thenAnswer((_) async => {});
        when(
          () => mockAuthLocalDataSource.cacheTokens(
            accessToken: any(named: 'accessToken'),
        ...
        expect(
          ...
          Right(
            SignInEntity(
              message: signInResponseModel.message,
              user: signInResponseModel.user.toEntity(),
            ),
          ),
        );
        ...
        verify(
          () =>
              mockAuthLocalDataSource.cacheUser(user: signInResponseModel.user),
        ).called(1);
        verify(
          () => mockAuthLocalDataSource.cacheTokens(
  ...
  group('sign out repository test', () {
    ...
    test(
      ...
      () async {
        ...
        when(
          () => mockAuthLocalDataSource.clearCache(),
        ).thenAnswer((_) async => {});
  ...
  group('update profile repository test', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          () => mockAuthRemoteDataSource.updateProfile(params),
        ).thenAnswer((_) async => resultMatcher);
  ...
  group('get refresh token repository test', () {
    test(
      'should return Right(String?) when local call is successful',
      () async {
        // arrange
        when(
          () => mockAuthLocalDataSource.getRefreshToken(),
